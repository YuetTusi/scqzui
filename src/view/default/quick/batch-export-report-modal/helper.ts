...
import { ITreeNode, IzTreeObj } from '@/type/ztree';
...
const toTreeData = async (caseName: string, devices: DeviceType[]) => {
    ...
    let deviceNodes = await mapDeviceToTree(clone(devices));
...
const mapDeviceToTree = async (devices: DeviceType[]) => {
    ...
    for (let i = 0; i < devices.length; i++) {
        ...
        if (hasReport) {
            ...
            nodes = nodes.concat([
                {
                    name: `${onlyName}${mode === DataMode.ServerCloud ? '-云取' : ''}（${mobileHolder}）${dayjs(fetchTime).format('YYYY-MM-DD HH:mm:ss')}`,
                    deviceId: _id,
                    phonePath,
                    mobileHolder,
                    mobileName,
