...
export default {
  ...
  initAllDeviceParseState({ dispatch }: SubscriptionAPI) {
    ...
    dispatch({
      type: 'updateAllDeviceParseState',
      payload: ParseState.NotParse,
    });
  },
  /**
   * 读取conf配置文件、JSON等
      ...
      try {
        ...
        if (res.status >= 200 && res.status < 300) {
          ...
          if (code === 0) {
            ...
            logger.error(
              `云取证应用数据获取失败 @model/default/app-set/subscriptions/validCloudAppMd5: request()查询结果错误`
            );
          }
        ...
        logger.error(
          `云取证应用数据获取失败 @model/default/app-set/subscriptions/validCloudAppMd5: ${error.message}`
        );
        hide();
        message.error('云取证应用数据获取失败');
      }
