...
const CodeItem: FC<CodeItemProps> = ({
		...
		if (message && message.length > 0) {
			...
			switch (type) {
				...
				case SmsMessageType.Warning:
					return <strong style={{ color: '#f5222d' }}>{content}</strong>;
				case SmsMessageType.Important:
					return <strong style={{ color: '#f9ca24' }}>{content}</strong>;
				default:
					return <strong style={{ color: '#fff' }}>{content}</strong>;
			}
		} else {
	...
	return <div className="capp-row">
		...
		<div className="fn-input-panel">
			...
			<Button
				onClick={() =>
					humanVerifyDataHandle(app.humanVerifyData, app.isUrl, app.m_strID, appDesc)
				}
